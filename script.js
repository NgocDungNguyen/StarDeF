        ...
        if (currentTime - lastShootTime >= shootDelay) {
            switch (this.bulletType) {
                ...
                case 'auto':
                    ...
                    break;
                case 'laser':
                    const laserWidth = 3 + (this.bulletCount - 1) * 2; // Increase width based on bulletCount
                    bullets.push(new Laser(this.x, this.y, laserWidth));
                    break;
            }
...
class Boss {
    constructor() {
        ...
        this.maxHp = this.hp;
        this.teleportCooldown = level === 3 ? 300 : 180;
        this.shootCooldown = level === 3 ? 180 : 120 - level * 10;
    }
...
function gameUpdate() {
    ...
    if (isBossBattle) {
        ...
        bullets.forEach((bullet, bulletIndex) => {
            ...
            } else if (collision(boss, bullet) || (bullet instanceof Laser && laserCollision(bullet, boss))) {
        ...
        bullets.forEach((bullet, bulletIndex) => {
            ...
            enemies.forEach((enemy, enemyIndex) => {
                if (collision(bullet, enemy) || (bullet instanceof Laser && laserCollision(bullet, enemy))) {
...
function laserCollision(laser, obj) {
    return (
        laser.x - laser.width / 2 < obj.x + obj.width &&
        laser.x + laser.width / 2 > obj.x &&
        obj.y + obj.height > 0
    );
}
